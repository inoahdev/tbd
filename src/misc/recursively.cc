...
enum recursively_create_directories_from_file_path_inner_options : uint64_t {
    recursively_create_directories_from_file_path_inner_option_create_last_as_file      = 1 << 0,
    recursively_create_directories_from_file_path_inner_option_create_last_as_directory = 1 << 1
};
...
char *recursively_create_directories_from_file_path_inner(char *path, char *begin, char *end, char *slash, uint64_t options, int *last_descriptor = nullptr) {
    ...
    do {
        if (path_component_end == end) {
            if (options & recursively_create_directories_from_file_path_inner_option_create_last_as_file) {
            ...
            if (options & recursively_create_directories_from_file_path_inner_option_create_last_as_directory) {
...
char *recursively_create_directories_from_file_path_creating_last_as_file(char *path, size_t index, int *last_descriptor) {
    ...
    while (path_component_begin != path_reverse_iter) {
        ...
        *path_component_end = '\0';
        ...
        if (access(path, F_OK) != 0) {
            *path_component_end = path_component_end_char;
            recursively_create_directories_from_file_path_inner(path, path_component_begin, path_reverse_iter, path_component_end, recursively_create_directories_from_file_path_inner_option_create_last_as_file, last_descriptor);
...
char *recursively_create_directories_from_file_path_creating_last_as_directory(char *path, size_t index) {
    ...
    while (path_component_begin != path_reverse_iter) {
        ...
        *path_component_end = '\0';
        ...
        if (access(path, F_OK) != 0) {
            *path_component_end = path_component_end_char;
            recursively_create_directories_from_file_path_inner(path, path_component_begin, path_reverse_iter, path_component_end, recursively_create_directories_from_file_path_inner_option_create_last_as_directory);
