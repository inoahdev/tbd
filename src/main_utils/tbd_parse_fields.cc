...
namespace main_utils {
    ...
    enum utils::tbd::objc_constraint parse_objc_constraint_from_argument(int &index, int argc, const char *argv[]) {
        ...
        const auto objc_constraint = utils::tbd::objc_constraint_from_string(argv[index]);
        if (objc_constraint == utils::tbd::objc_constraint::none && strcmp(argv[index], "none") != 0) {
    ...
    enum utils::tbd::platform parse_platform_from_argument(int &index, int argc, const char *argv[]) {
        ...
        const auto platform = utils::tbd::platform_from_string(argument);

        if (platform == utils::tbd::platform::none) {
