//
//  src/mach-o/utils/symbols/table_64/data.cc
//  tbd
//
...
namespace macho::utils::symbols::table_64 {
    ...
    data::creation_result data::create(const container &container, const symtab_command &symtab, const options &options) noexcept {
        ...
        if (!::utils::range::is_valid_size_based_range(container.base, symbol_entry_table_location)) {
            return creation_result::invalid_symbol_entry_table;
        }
        ...
        if (container.is_big_endian()) {
            ...
            if (!flags.bits.is_big_endian) {
                swap::symbol_table::entries_64(symbol_entry_table, symbol_table_entry_count);
            }
