...
namespace macho {
    void swap_uint16(uint16_t &uin16) {
        uin16 = ((uin16 >> 8) & 0x00ff) | ((uin16 << 8) & 0xff00);
    }

    void swap_uint32(uint32_t &uin32) {
         uin32 = ((uin32 >>  8) & 0x00ff00ff) | ((uin32 <<  8) & 0xff00ff00);
         uin32 = ((uin32 >> 16) & 0x0000ffff) | ((uin32 << 16) & 0xffff0000);
    }

    void swap_uint64(uint64_t &uin64) {
        uin64 = (uin64 & 0x00000000ffffffff) << 32 | (uin64 & 0xffffffff00000000) >> 32;
        uin64 = (uin64 & 0x0000ffff0000ffff) << 16 | (uin64 & 0xffff0000ffff0000) >> 16;
        uin64 = (uin64 & 0x00ff00ff00ff00ff) << 8  | (uin64 & 0xff00ff00ff00ff00) >> 8;
    }
    ...
    void swap_fat_arch(architecture &arch) {
        swap_uint32(*(uint32_t *)&arch.cputype);
        swap_uint32(*(uint32_t *)&arch.cpusubtype);
        swap_uint32(arch.offset);
        swap_uint32(arch.size);
        swap_uint32(arch.align);
    }

    void swap_fat_arch_64(architecture_64 &arch) {
        swap_uint32(*(uint32_t *)&arch.cputype);
        swap_uint32(*(uint32_t *)&arch.cpusubtype);
        swap_uint64(arch.offset);
        swap_uint64(arch.size);
        swap_uint32(arch.align);
    }

    void swap_fat_archs(architecture *archs, uint32_t count) {
        while (count) {
            swap_fat_arch(*archs);
    ...
    void swap_fat_archs_64(architecture_64 *archs, uint32_t count) {
        while (count) {
    ...
    void swap_nlist_64(nlist_64 &nlist) {
        swap_uint32(nlist.n_un.n_strx);
    ...
    void swap_nlists(nlist *nlists, uint32_t count) {
        while (count) {
            swap_nlist(*nlists);
    ...
    void swap_nlists_64(nlist_64 *nlists, uint32_t count) {
        while (count) {
            swap_nlist_64(*nlists);
    ...
    void swap_twolevel_hints_command(twolevel_hints_command &twolevel_hints) {
        swap_load_command(twolevel_hints);
    ...
    void swap_encryption_info_command(encryption_info_command &encryption_info) {
        swap_load_command(encryption_info);
