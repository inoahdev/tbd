...
namespace utils {
    ...
    tbd::write_result
    tbd_check_prerequisites_before_writing(const tbd &tbd,
                                           const tbd::write_options &options,
    ...
    template <typename T>
    tbd::write_result tbd_write_with_export_groups_to(const tbd &tbd,
                                                      const stream_helper<T> &stream,
                                                      const tbd::write_options &options,
                                                      const tbd::version &version,
    ...
    template <typename T>
    tbd::write_result write_exports_to_stream(const tbd &tbd,
                                              const stream_helper<T> &stream,
                                              const tbd::write_options &options,
                                              const tbd::version &version,
                                              const std::vector<tbd::export_group> &groups) noexcept;
    ...
    template <typename T>
    tbd::write_result write_group_to_stream(const stream_helper<T> &stream,
                                            FILE *file,
                                            const tbd::export_group &group,
    ...
    template <typename T>
    bool write_clients_array_to_stream(const stream_helper<T> &stream,
                                       const std::vector<tbd::client>::const_iterator &begin,
                                       const std::vector<tbd::client>::const_iterator &end,
                                       const tbd::version &version,
                                       uint64_t architectures) noexcept;
    
    template <typename T>
    bool write_reexports_array_to_stream(const stream_helper<T> &stream,
                                         const std::vector<tbd::reexport>::const_iterator &begin,
                                         const std::vector<tbd::reexport>::const_iterator &end,
                                         uint64_t architectures) noexcept;

    template <typename T>
    bool write_normal_symbols_array_to_stream(const stream_helper<T> &stream,
                                              const std::vector<tbd::symbol>::const_iterator &begin,
                                              const std::vector<tbd::symbol>::const_iterator &end,
                                              uint64_t architectures) noexcept;

    template <typename T>
    bool write_objc_class_symbols_array_to_stream(const stream_helper<T> &stream,
                                                  const std::vector<tbd::symbol>::const_iterator &begin,
                                                  const std::vector<tbd::symbol>::const_iterator &end,
                                                  uint64_t architectures) noexcept;

    template <typename T>
    bool write_objc_ivar_symbols_array_to_stream(const stream_helper<T> &stream,
                                                 const std::vector<tbd::symbol>::const_iterator &begin,
                                                 const std::vector<tbd::symbol>::const_iterator &end,
                                                 uint64_t architectures) noexcept;

    template <typename T>
    bool write_weak_symbols_array_to_stream(const stream_helper<T> &stream,
                                            const std::vector<tbd::symbol>::const_iterator &begin,
                                            const std::vector<tbd::symbol>::const_iterator &end,
                                            uint64_t architectures) noexcept;

    std::vector<tbd::symbol>::const_iterator
    next_iterator_for_symbol(const std::vector<tbd::symbol>::const_iterator &begin,
                             const std::vector<tbd::symbol>::const_iterator &end,
                             uint64_t architectures,
                             enum tbd::symbol::type type) noexcept;
    ...
    tbd::write_result
    tbd::write_with_export_groups_to(int descriptor,
    ...
    tbd::write_result
    tbd::write_with_export_groups_to(FILE *file,
                                     const write_options &options,
                                     const version &version,
                                     const std::vector<export_group> &groups) const noexcept
    {
                                           ...
                                           const std::vector<tbd::export_group> &groups)
    {
        if (!options.ignore_architectures) {
            ...
            const auto architecture_info_size = macho::get_architecture_info_table_size();
    ...
    bool write_architectures_to_stream(const stream_helper<T> &stream, uint64_t architectures, bool dash) noexcept {
        ...
        const auto architecture_info_max_index = macho::get_architecture_info_table_size() - 1;
        ...
        auto architecture_info = macho::architecture_info_from_index(first_architecture_index);
        if (dash) {
        ...
        for (auto index = first_architecture_index; index != architecture_info_max_index; index++, architecture_index_bit <<= 1) {
            ...
            architecture_info = macho::architecture_info_from_index(index);
    ...
    template <typename T>
    tbd::write_result write_group_to_stream(const tbd &tbd,
                                            const stream_helper<T> &stream,
                                            const tbd::export_group &group,
                                            const tbd::write_options &options,
                                            const tbd::version &version) noexcept
    {
    ...
    bool write_clients_array_to_stream(const stream_helper<T> &stream,
                                       const std::vector<tbd::client>::const_iterator &begin,
                                       const std::vector<tbd::client>::const_iterator &end,
                                       const tbd::version &version, uint64_t architectures) noexcept
    {
        switch (version) {
